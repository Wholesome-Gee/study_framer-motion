...
const pathVariant = {
  start:{
    fill: "rgba(0,0,0,0)",  // svg -> path는 fill 속성을 갖고있다. fill은 svg의 색상을 나타낸다
    pathLength: 0   // svg -< path는 pathLength 속성을 갖고있다. pathLength는 svg의 테두리가 그려진 정도를 나타낸다.
  },
  end:{
    fill: "rgba(0,0,0,1)",
    pathLength: 1,  
...
// svg는 path를 갖고있고 path는 stroke속성을 갖고있다. stroke는 svg의 테두리 색상을 나타낸다
// svg는 path를 갖고있고 path는 strokeWidth속성을 갖고있다. strokeWidth는 svg의 테두리 두께를 나타낸다
export default function App() {
  return (
    <Container>
      <Svg 
...
### #8.13
**📗\<AnimatePresence>를 사용하여 슬라이드 구현하기 part.2**
...
**📗motion의 layout속성으로 요소의 layout변화에 자동으로 애니메이션 부여하기**
**📗motion의 layoutId속성으로 서로 다른 요소의 애니메이션을 연결하기**
```jsx
import { motion } from "motion/react"
import { useState } from "react"
import styled from "styled-components"
...
**📗layoutId를 사용하여 OverLayout 구현하기 part.1**
- OverLayout 만들기의 기본 원리 이해
```jsx
import { AnimatePresence, motion } from "motion/react";
import { useState } from "react";
import styled from "styled-components";
